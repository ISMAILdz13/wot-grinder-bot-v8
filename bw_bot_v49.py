#!/usr/bin/env python3
"""WoT Bot v49 — Partition bit encoding (THE FIX)

ROOT CAUSE: Our solver used HALFSIZE offset (U in [1,HALFSIZE], V in [HALFSIZE+1,2*HALFSIZE]).
This makes the ck graph find BIPARTITE CYCLES (shared nodes) instead of CUCKOO CYCLES (V_i = U_{i+1}).

FIX: Use Tromp's partition bit encoding:
  U = sipnode * 2     (even, partition bit 0)
  V = sipnode * 2 + 1 (odd, partition bit 1)
Both in [0, 2*NODEMASK+1] = [0, SIZE-1]
The ck graph now finds Cuckoo cycles where V_i >> 1 = U_{i+1} >> 1.
"""
...
# V = sipnode * 2 + 1 (odd,  in [1, 2*NODEMASK+1])
# Both in [0, SIZE-1]
# ck graph finds Cuckoo cycles (V_i >> 1 = U_{i+1} >> 1)
...
def solve_cuckoo(header, easiness):
    ...
    for n in range(easiness):
        u0 = sipnode(ctx, n, 0) * 2       # even (partition bit 0)
        v0 = sipnode(ctx, n, 1) * 2 + 1   # odd  (partition bit 1)
        u = ck[u0]; v = ck[v0]
        if u == v0 or v == u0: continue    # self-loop
...
def find_sol(ctx, us, nu, vs, nv, easiness):
    ...
    sol = []
    for n in range(easiness):
        u = sipnode(ctx,n,0)*2; v = sipnode(ctx,n,1)*2+1  # partition bit encoding
        if (u,v) in cycle:
...
def verify_tromp(key_prefix_bytes, solution):
    """REAL Tromp verification from cuckoo.h:
    uvs use raw sipnode (without partition bit), both in [0, NODEMASK]
    Match values at same parity, follow cycle for PROOFSIZE steps
    """
    ...
    for n in range(PROOFSIZE):
        ...
        uvs[2*n] = sipnode(ctx, solution[n], 0)
        uvs[2*n+1] = sipnode(ctx, solution[n], 1)
    ...
    for n in range(PROOFSIZE):
        ...
        xor1 ^= uvs[2*n+1]
    if xor0 | xor1:
        return False, f"XOR check failed: {xor0}, {xor1}"
    ...
    while True:
        ...
        while k != i:
            if uvs[k] == uvs[i]:
                if j != i: return False, f"Branch at step {n}"
                j = k
            k = (k + 2) % (2 * PROOFSIZE)
        if j == i: return False, f"Dead end at step {n}"
        i = j ^ 1
        n += 1
        if i == 0: break
    
    if n == PROOFSIZE:
        # Count unique U and V values
        u_vals = set(uvs[0::2])
        v_vals = set(uvs[1::2])
        return True, f"VALID Cuckoo cycle! {n} steps, {len(u_vals)} U-values, {len(v_vals)} V-values"
    return False, f"Short cycle: {n} steps"
...
def main():
    ...
    for attempt in range(1, 16):
        ...
        if solution and len(solution) == 42:
            ...
            valid, msg = verify_tromp(key_prefix, solution)
            print(f"    Tromp: {valid} — {msg}")
            if valid:
                break
            print("    Tromp failed, sending anyway...")
            break
        else:
            print("    No 42-cycle, reopening socket...")
            sock.close()
            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            sock.settimeout(15)
            sock.sendto(build_ping(rid), SERVER)
