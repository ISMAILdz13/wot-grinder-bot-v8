#!/usr/bin/env python3
"""WoT Bot v37 — C++ BigWorld LogOnParams format (u32 length, no context)

v36 used wg-toolkit-rs format (packed_u24 + context) → "Could not destream"
C++ BigWorld uses: [flags(1B)] [username(u32+data)] [password(u32+data)] [encKey(u32+data)] [digest?(16B)] [nonce(u32)]
No context field. u32 length prefix (not packed_u24).
"""
...
def pack_u32_str(s):
    """C++ BigWorld string: u32 length + data"""
    b = s.encode() if isinstance(s, str) else s
...
def build_unencrypted_login_body(protocol):
    """Unencrypted first login (just to get Cuckoo challenge)"""
    bf = os.urandom(16)
    # Use packed_u24 format (server doesn't parse this, just issues challenge)
...
def build_encrypted_login_body(protocol, bf_key, rsa_key_pem, use_digest=False):
    """RSA-encrypted login with C++ BigWorld LogOnParams format
    
    C++ format: [flags(1B)] [username(u32+data)] [password(u32+data)] [encKey(u32+data)] [digest(16B)?] [nonce(u32)]
    u32 length prefix for strings (NOT packed_u24)
    No context field
    """
    logon = struct.pack("<B", 1 if use_digest else 0)  # flags
    logon += pack_u32_str("guest")                     # username
    logon += pack_u32_str("")                           # password
    logon += pack_u32_str(bf_key)                       # encryptionKey
    if use_digest:
        logon += b"\x00" * 16                           # digest (zeros)
    logon += struct.pack("<I", 0)                       # nonce
    
    # RSA-OAEP-SHA1 encrypt
    key = RSA.importKey(rsa_key_pem)
    cipher = PKCS1_OAEP.new(key, hashAlgo=SHA1)
    encrypted = cipher.encrypt(logon)  # 256 bytes
...
# Format combinations to try
FORMATS = [
    ("KEY_WOT", KEY_WOT, False, "u32 len, no context, flags=0 (no digest)"),
    ("KEY_WOT", KEY_WOT, True,  "u32 len, no context, flags=1 (with digest)"),
    ("KEY_BW",  KEY_BW,  False, "u32 len, no context, flags=0 (no digest)"),
    ("KEY_BW",  KEY_BW,  True,  "u32 len, no context, flags=1 (with digest)"),
]

def run(server="login.p1.worldoftanks.eu", port=20016, timeout=10, max_attempts=20):
    PROTOCOL = 285278213
    print(f"\n{'='*55}")
    print(f"  WoT Bot v37 — C++ LogOnParams (u32 len)")
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            except: print(f"    PING failed"); sock.close(); continue
        ...
        # Try current format combo
        key_name, key_pem, use_digest, fmt_desc = FORMATS[fmt_idx % len(FORMATS)]
        fmt_idx += 1
        
        print(f"    Format: {key_name} | {fmt_desc}")
        enc_login = build_encrypted_login_body(PROTOCOL, bf_key, key_pem, use_digest)
        ...
        try:
            ...
            if result:
                ...
                if status == 0x01:
                    ...
                    print(f"    -> Invalid User — RSA + format ACCEPTED! Need real WG creds")
                    sock.close(); return
                elif status == 0x48:
                    print(f"    -> Invalid Password — RSA + format ACCEPTED!")
                    sock.close(); return
                elif b"destream" in extra:
                    print(f"    -> Still can't destream — wrong RSA key or format")
                elif b"Unencrypted" in extra:
                    print(f"    -> Still unencrypted — encryption not detected")
                else:
    ...
    print(f"\nExhausted {max_attempts} attempts, tried {fmt_idx} format combos.")
