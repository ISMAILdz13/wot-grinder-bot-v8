#!/usr/bin/env python3
"""WoT Bot v41 — Clean single-shot, mirrors ping_compare exactly"""
...
# === Packet functions (verified by ping_compare) ===
def _prefix(raw):
...
def build_login_body(protocol, bf_key, encrypted=False, rsa_key_pem=None, context=""):
    ...
    logon += pack_str(context)           # context
...
def main():
    print(f"\n{'='*55}")
    print(f"  WoT Bot v41 — Clean single-shot")
    ...
    # === Step 0: PING (exactly like ping_compare) ===
    print("[0] PING...", end=" ", flush=True)
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.settimeout(5)
    rid = 1
    pkt = build_ping(rid)
    print(f"({len(pkt)}B: {pkt.hex()})", end=" ", flush=True)
    ...
    # === Step 1: Unencrypted login → get Cuckoo challenge ===
    print("[1] Login request (unencrypted)...")
    ...
    print(f"    Status: 0x{status:02X}, rid: {reply_rid}")
    ...
    if solution is None or len(solution) != 42:
        print(f"    No 42-cycle found")
        sock.close(); return
    print(f"    Solution: {len(solution)} nonces, {duration:.1f}s")

    # === Step 3: Send ChallengeResponse (separate packet, MESSAGE) ===
    print("[3] Sending ChallengeResponse...")
    cr_body = build_cr_body(duration, key_prefix, solution)
    cr_elem = build_message_v16(0x03, cr_body)
    cr_pkt = build_packet(cr_elem, first_req=None)
    print(f"    Packet: {len(cr_pkt)}B")
    sock.sendto(cr_pkt, SERVER)

    # === Step 4: Send encrypted LoginRequest (separate packet, REQUEST) ===
    print("[4] Sending encrypted LoginRequest...")
    # Try KEY_BW first (server could decrypt it before)
    for key_name, key_pem in [("KEY_BW", KEY_BW), ("KEY_WOT", KEY_WOT)]:
        ...
        print(f"    Packet: {len(login_pkt)}B")
        sock.sendto(login_pkt, SERVER)
        rid += 1
