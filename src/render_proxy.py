...
def send_packet():
    ...
    # Support custom server, default to p1 with p2 fallback
    server = data.get("server", "login.p1.worldoftanks.eu")
    port = data.get("port", 20016)
    servers_to_try = [(server, port)]
    if server == "login.p1.worldoftanks.eu":
        servers_to_try.append(("login.p2.worldoftanks.eu", port))
    
    for wot_server in servers_to_try:
        sock = get_socket()
        ...
        try:
            ...
            while True:
                try:
                    ...
                    break
            if responses:
                return jsonify({"ok": True, "responses": responses, "count": len(responses), "server": f"{wot_server[0]}:{wot_server[1]}"})
            # Try next server if this one timed out
            try: sock.close()
            except: pass
            _sock = None
            ...
            _sock = None
    return jsonify({"ok": False, "error": "timeout", "tried": [f"{s[0]}:{s[1]}" for s in servers_to_try]})
