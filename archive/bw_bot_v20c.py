...
import socket, struct, os, sys, time, hashlib, json, urllib.request, urllib.parse, http.cookiejar, ssl
...
def siphash24(data, key):
    ...
    def sipround(v0, v1, v2, v3):
        ...
        return v0, v1, v2, v3
    k0 = struct.unpack("<Q", key[:8])[0]; k1 = struct.unpack("<Q", key[8:])[0]
    v0 = k0 ^ 0x736f6d6570736575; v1 = k1 ^ 0x646f72616e646f6d
    v2 = k0 ^ 0x6c7967656e657261; v3 = k1 ^ 0x7465646279746573
    pos = 0
    while pos + 8 <= len(data):
        m = struct.unpack("<Q", data[pos:pos+8])[0]; v3 ^= m
        v0,v1,v2,v3 = sipround(v0,v1,v2,v3); v0,v1,v2,v3 = sipround(v0,v1,v2,v3); v0 ^= m; pos += 8
    remaining = data[pos:]; last = len(data) & 0xFF
    last_block = remaining + b'\x00' * (7 - len(remaining)) + bytes([last])
    m = struct.unpack("<Q", last_block)[0]; v3 ^= m
...
def solve_cuckoo(challenge_key, max_nonce=500000):
    for nonce in range(max_nonce):
        h = siphash24(struct.pack("<I", nonce), challenge_key[:16])
        if (h & 0xFFFFFFFF00000000) == 0: return nonce
    return None
...
def wg_login(email, password):
    ...
    try: opener.open(urllib.request.Request("https://eu.wargaming.net/id/api/v2/settings/"), timeout=10)
    except Exception as e: print(f"  Settings: {e}"); return None
    csrf = None
    for cookie in cookies:
        if 'csrftoken' in cookie.name.lower(): csrf = cookie.value; break
    if not csrf: print("  No CSRF"); return None
    print(f"  CSRF: {csrf[:20]}...")
    try:
        resp = opener.open(urllib.request.Request("https://eu.wargaming.net/id/signin/challenge/?feature=authentication_basic&type=pow"), timeout=10)
        challenge_data = json.loads(resp.read())
    except Exception as e: print(f"  Challenge: {e}"); return None
    stamp = challenge_data.get('stamp', ''); complexity = challenge_data.get('complexity', 3)
    ...
    for counter in range(100000):
        ...
        if h.hex()[:complexity] == '0' * complexity:
            print(f"  POW solved! counter={counter}")
            pow_value = data; break
    else: print("  POW failed"); return None
    post_data = urllib.parse.urlencode({'login': email, 'password': password, 'remember': 'on', 'pow': pow_value}).encode()
    req = urllib.request.Request("https://eu.wargaming.net/id/signin/process/?type=pow", data=post_data, headers={
        'X-CSRFToken': csrf, 'Content-Type': 'application/x-www-form-urlencoded', 'Referer': 'https://eu.wargaming.net/id/signin/'})
    try:
        ...
        for _ in range(10):
            ...
            try:
                ...
                if status.get('status') == 'ok': print(f"  Login OK!"); return status
                elif status.get('status') == 'error': print(f"  Error: {status}"); return None
            except: pass
    except Exception as e: print(f"  Submit: {e}")
    return None
...
def run(server="login.p1.worldoftanks.eu", port=20016, timeout=5):
    ...
    try:
        data, _ = sock.recvfrom(4096); print(f"    PING OK"); rid += 1
    except socket.timeout: print(f"    PING timeout"); sock.close(); return
        ...
        try:
            ...
            if r.get("type") == "CHALLENGE":
                ...
                if len(challenge_data) >= 5:
                    payload = challenge_data[5:]
                    print(f"      Payload ({len(payload)}B): {payload[:100].hex()}")
                    if len(payload) >= 4:
                        duration = struct.unpack("<f", payload[:4])[0]
                        print(f"      Duration: {duration}s")
                        cuckoo_key = payload[4:4+16] if len(payload) >= 20 else payload
                        print(f"      Cuckoo key: {cuckoo_key.hex()}")
                        print(f"      Solving Cuckoo PoW...")
                        solution = solve_cuckoo(cuckoo_key)
                        if solution is not None:
                            print(f"      Solution: {solution}")
                            challenge_resp = struct.pack("<f", duration) + pack_str(cuckoo_key) + struct.pack("<I", solution)
                            ...
                            try:
                                ...
                                print(f"      <- {r2}")
                                rid += 1
                                if r2.get("type") in ("SUCCESS", "CHALLENGE"):
                                    print(f"      Challenge accepted! Sending LoginRequest...")
                                    bf = os.urandom(16)
                                    ...
                                    try:
                                        ...
                                        print(f"      <- {r3}")
                                    except socket.timeout: print(f"      Login timeout")
                            except socket.timeout: print(f"      ChallengeResponse timeout")
                        else: print(f"      Cuckoo not solved")
                break
        except socket.timeout: print(f"      timeout"); rid += 1
        ...
        if auth:
            ...
            for proto in [51, 52, 55]:
                ...
                try:
                    ...
                    print(f"      <- {r}")
                    rid += 1
                    if r.get("type") == "CHALLENGE": print(f"      CHALLENGE!"); break
                except socket.timeout: print(f"      timeout"); rid += 1
    else:
