#!/usr/bin/env python3
"""WoT Bot v43 — Test BOTH RSA padding modes (OAEP-SHA1 vs PKCS1v15)

v42 got "destream" — could be wrong key OR wrong padding.
BigWorld C++ might use PKCS#1 v1.5 (default OpenSSL) not OAEP.
Test all 4 combos: KEY_BW×OAEP, KEY_BW×PKCS1, KEY_WOT×OAEP, KEY_WOT×PKCS1
"""
...
def build_login_body(protocol, bf_key, rsa_key_pem, context, padding="oaep"):
...
def main():
    print(f"\n{'='*55}")
    print(f"  WoT Bot v43 — Test OAEP vs PKCS1v15")
    print(f"  4 combos: BW×OAEP, BW×PKCS1, WOT×OAEP, WOT×PKCS1")
    print(f"{'='*55}\n")
    ...
    logon = struct.pack("<B", 0) + pack_str("guest") + pack_str("") + pack_str(bf_key) + pack_str("") + struct.pack("<I", random.randint(1, 0xFFFFFFFF))
    ...
    # CR element (shared for all combos)
    ...
    combos = [
        ("KEY_BW",  KEY_BW,  "oaep"),
        ("KEY_BW",  KEY_BW,  "pkcs1"),
        ("KEY_WOT", KEY_WOT, "oaep"),
        ("KEY_WOT", KEY_WOT, "pkcs1"),
    ]

    for key_name, key_pem, padding in combos:
        print(f"\n[3] {key_name} × {padding.upper()}...")
        enc_login = build_login_body(PROTOCOL, bf_key, key_pem, "eu_1.19.1_4", padding)
        ...
        try:
            ...
            if result:
                ...
                if status == 0x01: print("    === LOGIN SUCCESS! ==="); sock.close(); return
                    ...
                    if b"destream" in extra: print("    -> destream (wrong key or padding)")
